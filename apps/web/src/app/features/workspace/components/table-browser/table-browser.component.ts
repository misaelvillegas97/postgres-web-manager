...
import { TableDataService, TableChange } from '../../../../core/services/table-data.service';
...
const FILTER_OPS = ['=', '!=', '<', '>', '<=', '>=', 'LIKE', 'ILIKE', 'IS NULL', 'IS NOT NULL', 'IN'];
...
@Component({
    ...
    <div class="tb">
      ...
      <div class="tb__toolbar">
        <div class="tb__breadcrumb">
          ...
          @if (totalCount() >= 0) {
            <span class="tb__count text-muted">{{ totalCount() | number }} rows</span>
          }
        </div>
        <div class="tb__actions">
          <button class="btn btn--ghost btn--sm" (click)="showFilterBar.set(!showFilterBar())" title="Toggle filters">
            ⚙ Filters{{ activeFilters().length ? ' (' + activeFilters().length + ')' : '' }}
          </button>
          <button class="btn btn--ghost btn--sm" (click)="addRow()" [disabled]="!pkColumns().length" title="Add new row">
            + Row
          </button>
          @if (pendingChanges().length) {
            ...
            <button class="btn btn--ghost btn--sm" (click)="discardAll()">Discard</button>
          }
          <button class="btn btn--ghost btn--sm" (click)="reload()" title="Refresh">↺</button>
        </div>
      </div>
        ...
        <div class="tb__filterbar">
          @for (f of activeFilters(); track $index) {
            <div class="filter-row">
              <select class="input input--sm" [ngModel]="f.column" (ngModelChange)="f.column = $event">
                @for (col of columns(); track col.name) {
                  <option [value]="col.name">{{ col.name }}</option>
                }
              </select>
              <select class="input input--sm" [ngModel]="f.operator" (ngModelChange)="f.operator = $event">
                @for (op of filterOps; track op) {
                  <option [value]="op">{{ op }}</option>
                }
              </select>
              @if (f.operator !== 'IS NULL' && f.operator !== 'IS NOT NULL') {
                <input class="input input--sm" [ngModel]="f.value" (ngModelChange)="f.value = $event" placeholder="value" />
              }
              <button class="btn btn--ghost btn--sm btn--icon" (click)="removeFilter($index)">×</button>
            </div>
          }
          <div class="filter-row">
            <button class="btn btn--ghost btn--sm" (click)="addFilter()">+ Add filter</button>
              ...
              <tr>
                ...
                @for (col of columns(); track col.name) {
                  <th
                    ...
                    [class.tb__col-header--sorted]="sortState()?.column === col.name"
                    [class.tb__col-pk]="col.isPk"
                    (click)="toggleSort(col.name)"
                    title="{{ col.name }} ({{ col.dataTypeName ?? col.dataTypeId }})"
                  >
                    @if (col.isPk) { <span class="pk-badge" title="Primary key">🔑</span> }
                    {{ col.name }}
                    @if (sortState()?.column === col.name) {
                      <span class="sort-icon">{{ sortState()!.direction === 'ASC' ? '↑' : '↓' }}</span>
                    }
                  </th>
                }
              </tr>
                ...
                >
                  <td class="tb__cell-action">
                    @if (!isDeleted(row)) {
                      ...
                      <button class="btn-icon" title="Restore row" (click)="undeleteRow(row)">↩</button>
                    }
                  </td>
                  @for (col of columns(); track col.name) {
                    <td
                    ...
                    >
                      @if (isEditing(row, col.name) && !isDeleted(row)) {
                        ...
                        <span class="cell-value">{{ formatCell(getCellValue(row, col.name)) }}</span>
                      }
                    </td>
                  }
                </tr>
      ...
      @if (totalCount() > 0) {
        <div class="tb__pagination">
          ...
          <div class="tb__page-controls">
            <button class="btn btn--ghost btn--sm btn--icon" (click)="goPage(1)" [disabled]="page() === 1">«</button>
            <button class="btn btn--ghost btn--sm btn--icon" (click)="goPage(page() - 1)" [disabled]="page() === 1">‹</button>
            <span class="tb__page-num">Page {{ page() }} / {{ totalPages() }}</span>
            <button class="btn btn--ghost btn--sm btn--icon" (click)="goPage(page() + 1)" [disabled]="page() >= totalPages()">›</button>
            <button class="btn btn--ghost btn--sm btn--icon" (click)="goPage(totalPages())" [disabled]="page() >= totalPages()">»</button>
            <select class="input input--sm" [ngModel]="pageSize()" (ngModelChange)="setPageSize($event)">
      ...
      @if (showPreviewModal()) {
        <div class="modal-backdrop" (click)="showPreviewModal.set(false)">
          <div class="modal" (click)="$event.stopPropagation()">
            <div class="modal__header">
              <h3>Review Changes</h3>
              <button class="btn btn--ghost btn--sm btn--icon" (click)="showPreviewModal.set(false)">×</button>
            </div>
            <div class="modal__body">
              @if (previewLoading()) {
                <div class="modal-state"><span class="spinner"></span> Generating preview…</div>
                  ...
                  @for (stmt of previewStatements(); track $index) {
                    <div class="preview-stmt">
                      ...
                      @if (stmt.params.length) {
                        <div class="preview-params text-muted">Params: {{ stmt.params | json }}</div>
                      }
                    </div>
                  }
                </div>
              } @else {
                <div class="modal-state text-muted">No SQL statements generated.</div>
              }
            </div>
            <div class="modal__footer">
              <button class="btn btn--ghost" (click)="showPreviewModal.set(false)">Cancel</button>
              <button
                class="btn btn--danger"
                (click)="applyChanges()"
                [disabled]="applyLoading() || !previewStatements().length"
              >
                @if (applyLoading()) { <span class="spinner"></span> Applying… }
                @else { Apply {{ pendingChanges().length }} change{{ pendingChanges().length !== 1 ? 's' : '' }} }
              </button>
            </div>
          </div>
        </div>
  ...
  styles: [`
    ...
    .tb__schema { color: var(--text-secondary); }
    .tb__sep { color: var(--text-muted); }
    .tb__table { font-weight: 600; color: var(--text-primary); }
    .tb__count { font-size: 11px; margin-left: 8px; }

    .tb__actions { display: flex; gap: 6px; align-items: center; }
    ...
    .tb__state--error { color: var(--danger); }
    ...
    .pk-badge { font-size: 10px; margin-right: 3px; }
    ...
    .tb__row {
      ...
      &--pending { background: rgba(91, 106, 245, 0.06); }
      &--new { background: rgba(76, 175, 125, 0.08); }
      &--deleted { background: rgba(235, 87, 87, 0.08); opacity: 0.6; }
    }
    ...
    .tb__cell {
      ...
      &--pk { color: var(--accent); font-weight: 500; }
      &--edited { background: rgba(91, 106, 245, 0.12); }
    }
    ...
    .btn-icon {
      ...
      &:hover { opacity: 1; background: var(--bg-hover); }
      &:disabled { cursor: not-allowed; opacity: 0.2; }
    }
    ...
    .tb__page-info { color: var(--text-secondary); }
    ...
    .tb__page-num { padding: 0 8px; color: var(--text-secondary); }
    ...
    .modal-state { text-align: center; padding: 24px; font-size: 13px; }
...
export class TableBrowserComponent implements OnChanges {
  ...
  pendingChanges = computed((): PendingChange[] => {
    ...
    for (const [rowKey, editsMap] of edits.entries()) {
      ...
      pkCols.forEach((pk) => { primaryKey[pk] = before[pk]; });
      ...
      if (Object.keys(after).length) {
        changes.push({
          id: `update-${rowKey}`,
          change: { type: 'update', schema: this.schema, table: this.table, primaryKey, after },
        });
      }
    }
    ...
    for (const rowKey of deleted) {
      ...
      if (!row) continue;
      const primaryKey: Record<string, unknown> = {};
      pkCols.forEach((pk) => { primaryKey[pk] = row[pk]; });
      changes.push({ id: `delete-${rowKey}`, change: { type: 'delete', schema: this.schema, table: this.table, primaryKey } });
    }

    // Inserts
    newRows.forEach((row, i) => {
      changes.push({ id: `insert-${i}`, change: { type: 'insert', schema: this.schema, table: this.table, after: row } });
    });
  ...
  private loadPkInfo() {
    this.metaSvc.getTableDetail(this.connectionId, this.schema, this.table).subscribe({
      next: (detail) => {
        const pkCols = detail.columns.filter((c) => c.isPrimaryKey).map((c) => c.name);
        this.pkColumns.set(pkCols);
        // Mark PK columns in column defs
        this.columns.update((cols) => cols.map((c) => ({ ...c, isPk: pkCols.includes(c.name) })));
      },
      error: () => {},
    });
  }
  ...
  loadPage() {
    ...
    this.tdSvc.read({
      ...
      filters: filters.length
        ? filters.map((f) => ({ column: f.column, operator: f.operator, value: this.parseFilterValue(f) }))
        : undefined,
    }).subscribe({
      next: (res) => {
        const pkCols = this.pkColumns();
        this.columns.set(res.columns.map((c) => ({ ...c, isPk: pkCols.includes(c.name) })));
  ...
  toggleSort(col: string) {
    ...
    if (current?.column === col) {
      this.sortState.set({ column: col, direction: current.direction === 'ASC' ? 'DESC' : 'ASC' });
    } else {
  ...
  deleteRow(row: Record<string, unknown>) {
    ...
    const key = this.rowKey(row);
    this._deleted.update((s) => { const next = new Set(s); next.add(key); return next; });
  }

  undeleteRow(row: Record<string, unknown>) {
    const key = this.rowKey(row);
    this._deleted.update((s) => { const next = new Set(s); next.delete(key); return next; });
  }

  addRow() {
    const empty: Record<string, unknown> = {};
    for (const col of this.columns()) { empty[col.name] = null; }
  ...
  openPreview() {
    ...
    this.tdSvc.previewChanges({ connectionId: this.connectionId, changes }).subscribe({
      next: (res) => {
  ...
  applyChanges() {
    ...
    const changes = this.pendingChanges().map((pc) => pc.change);
    this.tdSvc.applyChanges({ connectionId: this.connectionId, changes }).subscribe({
      next: (res) => {
        this.applyLoading.set(false);
        this.showPreviewModal.set(false);
        if (res.status === 'success') {
          this.toast.success(`Applied ${res.affectedRows} row change${res.affectedRows !== 1 ? 's' : ''}`);
          this.discardAll();
          this.loadPage();
        } else {
