    ...
    <div class="analyzer">
      ...
      @if (!result() && !loading() && !error()) {
        <div class="analyzer__empty">
          ...
          <p>
            Paste or select a query in the SQL Editor, then click
            <strong>Explain</strong>.
          </p>
          <p class="text-muted">
            Enable <em>ANALYZE</em> for real execution stats (runs the query).
          </p>
        </div>
      }
      ...
      @if (result() && !loading()) {
        ...
        <div class="analyzer__summary">
          <div class="summary-card">
            ...
            <div
              ...
              [class.text-warn]="totalCost() > 1000"
              [class.text-crit]="totalCost() > 10000"
            >
          ...
          @if (result()!.planningTimeMs !== undefined) {
            <div class="summary-card">
              ...
              <div class="summary-card__value">
                {{ result()!.planningTimeMs | number: '1.2-2' }}ms
              </div>
            </div>
          }
          @if (result()!.executionTimeMs !== undefined) {
            <div class="summary-card">
              ...
              <div
                class="summary-card__value"
                [class.text-warn]="(result()!.executionTimeMs ?? 0) > 500"
              >
          ...
          @if (
            ...
            <div class="summary-card">
              ...
              <div class="summary-card__value">
                {{
                  result()!.executionTimeMs! + result()!.planningTimeMs!
                    | number: '1.2-2'
                }}ms
              </div>
            </div>
        ...
        @if (warnings().length) {
          <div class="analyzer__warnings">
            @for (w of warnings(); track $index) {
              <div
                class="warning-pill"
                [class.warning-pill--crit]="w.severity === 'crit'"
              >
                <span>
                  @if (w.severity === 'crit') {
                    ⛔
        ...
        <div class="analyzer__tree">
          <div class="tree-header">
            ...
            <span class="sep">·</span>
            <button class="btn-link" (click)="collapseAll()">
              collapse all
            </button>
          </div>
          <div class="tree-body">
...
export class QueryAnalyzerComponent implements OnChanges {
  ...
  expandAllState = signal<boolean | null>(null);
  expandVersion = signal(0);
  ...
  run() {
    ...
    if (!sqlText) {
      this.toast.error('No SQL to explain');
      return;
    }
  ...
  private setAllExpanded(expanded: boolean) {
    this.expandAllState.set(expanded);
    this.expandVersion.update((version) => version + 1);
  }
  ...
  private collectWarnings(node: ExplainPlanNode, out: PlanWarning[]) {
    ...
    if (node.totalCost > 10_000) {
      out.push({
        type: 'high-cost',
        severity: 'crit',
        message: `High cost node: ${node.nodeType} (cost ${node.totalCost.toFixed(0)})`,
      });
    } else if (node.totalCost > 1_000) {
      out.push({
        type: 'high-cost',
        severity: 'warn',
        message: `Moderate cost: ${node.nodeType} (cost ${node.totalCost.toFixed(0)})`,
      });
    }
    ...
    if (
      ...
      (node.actualRows ?? node.planRows) > 1000
    ) {
      out.push({
        type: 'seq-scan',
        severity: 'warn',
        message: `Seq Scan on ${node.relation ?? 'table'} — consider adding an index`,
      });
    }
    ...
    if (node.actualRows !== undefined && node.planRows > 0) {
      ...
      if (factor > 10 || (factor < 0.1 && node.planRows > 10)) {
        out.push({
          type: 'bad-estimate',
          severity: 'warn',
          message: `Row estimate mismatch on ${node.nodeType}: planned ${node.planRows}, actual ${node.actualRows}`,
        });
      }
    }
    ...
    if ((node.sharedReadBlocks ?? 0) > 1000) {
      out.push({
        type: 'disk-read',
        severity: 'crit',
        message: `High disk reads: ${node.sharedReadBlocks} blocks on ${node.nodeType}`,
      });
    } else if ((node.sharedReadBlocks ?? 0) > 100) {
      out.push({
        type: 'disk-read',
        severity: 'warn',
        message: `Disk reads: ${node.sharedReadBlocks} blocks on ${node.nodeType}`,
      });
    }
