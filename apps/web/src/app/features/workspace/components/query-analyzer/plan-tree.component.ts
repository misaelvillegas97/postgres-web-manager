...
@Component({
  ...
  template: `
    <div class="plan-node" [class.plan-node--expanded]="expanded()">
      ...
      >
        <span
          class="plan-node__toggle"
          [class.plan-node__toggle--open]="expanded()"
          >›</span
        >

        <span
          class="plan-node__type"
          [class]="'node-type node-type--' + nodeClass()"
        >
          {{ node.nodeType }}
        </span>
        ...
        @if (node.relation) {
          <span class="plan-node__rel">
            @if (node.schema) {
              <span class="plan-node__schema">{{ node.schema }}.</span>
            }
            {{ node.relation }}
            @if (node.alias && node.alias !== node.relation) {
              <em class="plan-node__alias"> ({{ node.alias }})</em>
            }
          </span>
        }
        ...
        <!-- Cost bar -->
        <span
          class="plan-node__cost"
          [class.text-warn]="costLevel() === 'warn'"
          [class.text-crit]="costLevel() === 'crit'"
        >
          cost {{ node.startupCost | number: '1.0-1' }}..{{
            node.totalCost | number: '1.0-1'
          }}
        </span>

        <!-- Rows -->
        <span class="plan-node__rows">
          <span class="plan-node__rows-est" title="Estimated rows">{{
            node.planRows | number
          }}</span>
        ...
        @if (node.actualTotalTime !== undefined) {
          <span class="plan-node__time"
            >{{ node.actualTotalTime | number: '1.2-2' }}ms</span
          >
        }

        @if (costLevel() === 'crit') {
          <span class="badge badge--danger" title="High cost node"
            >⚠ critical</span
          >
          ...
          <div class="plan-stats">
            @if (node.actualTotalTime !== undefined) {
              <div class="plan-stat">
                ...
                <span class="plan-stat__value">
                  {{ node.actualStartupTime | number: '1.2-2' }}ms →
                  {{ node.actualTotalTime | number: '1.2-2' }}ms
                </span>
              </div>
            }
            ...
            @if (node.sharedReadBlocks !== undefined) {
              <div class="plan-stat">
                ...
                <span
                  class="plan-stat__value highlight-read"
  ...
  styles: [
    `
      ...
      .plan-node--warn > .plan-node__header {
        border-left: 3px solid #f5a623;
      }
      .plan-node--crit > .plan-node__header {
...
export class PlanTreeComponent implements OnChanges {
  ...
  @Input() maxCost = 0;
  @Input() expandAllState: boolean | null = null;
  @Input() expandVersion = 0;

  expanded = signal(true);

  ngOnChanges() {
    if (this.expandAllState !== null) {
      this.expanded.set(this.expandAllState);
    }
  }
  ...
  rowDeviation(): boolean {
    if (this.node.actualRows === undefined || this.node.planRows === 0)
      return false;
    return (
      this.node.actualRows > this.node.planRows * ROW_DEVIATION ||
      (this.node.actualRows < this.node.planRows / ROW_DEVIATION &&
        this.node.planRows > 10)
    );
  }
